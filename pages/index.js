  ...
  return (
    <div className="relative">
      ...
      <main className="md:pl-72">
        ...
        <Heading />
        <SelectDate data={data} time={time} setTime={setTime} />
        <SelectSlot time={time} />
...
function SelectDate({ data, setTime, time }) {
  console.log(data)
    ...
    <div className="mt-6 pl-8">
      ...
      <div className=" mt-2 flex gap-2  overflow-x-auto">
        {data.map((data, index,) => (
          ...
          >
            <DateBox
              // className={time && 'bg-sky-800 text-white'}
              time={time}
...
function SlotBtn({ hours }) {
  return (
    <button className="tex rounded-md border-2 border-sky-800 px-3 py-1.5 font-medium uppercase text-sky-800 ">
      {hours} pm - {hours + 1} pm
    </button>
  )
}
